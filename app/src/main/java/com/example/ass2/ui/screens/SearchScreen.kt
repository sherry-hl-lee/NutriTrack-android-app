...
fun SearchScreen(
    ...
    val filteredMeals = meals.filter { meal ->
        val matchQuery = meal.name.contains(query, ignoreCase = true)

        val matchType =
            selectedTypes.isEmpty() || selectedTypes.contains(meal.mealType)

        val matchCalories =
            !lowCalorieOnly || meal.calories < 500
